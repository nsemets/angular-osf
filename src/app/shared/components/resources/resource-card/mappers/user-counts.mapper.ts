...
export function MapUserCounts(
  response: UserCountsResponse,
): UserRelatedDataCounts {
  return {
    projects: response.data?.relationships?.nodes?.links?.related?.meta?.count,
    registrations:
      response.data?.relationships?.registrations?.links?.related?.meta?.count,
    preprints:
      response.data?.relationships?.preprints?.links?.related?.meta?.count,
